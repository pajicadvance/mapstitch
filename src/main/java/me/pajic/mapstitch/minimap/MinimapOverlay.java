...
public class MinimapOverlay {
	...
	private static final ResourceLocation BACKGROUND_TEXTURE = ResourceLocation.withDefaultNamespace("container/cartography_table/map");
	private static int noMapTextTimer = 100;

	public static void render(GuiGraphics graphics) {
		if (
				MC.player != null && MC.level != null && !MC.options.hideGui && ModUtil.hasCompass(MC)
				&& !MC.gui.getDebugOverlay().showDebugScreen() && !(MC.screen instanceof WorldMapScreen)
		) {
			...
			if (!atlas.isEmpty()) {
				...
				switch (ModConfigHolder.options().minimapBackground) {
					...
					case CLEAR -> graphics.fill(-4, -4, 132, 132, FastColor.ARGB32.color(
							FastColor.as8BitChannel(ModConfigHolder.options().minimapBackgroundOpacity / 100F),
							0, 0, 0
					));
				}
				...
				if (id != -1) {
					...
					if (mapData != null) {
						MC.gameRenderer.getMapRenderer().render(graphics.pose(), graphics.bufferSource(), mapId, mapData, true, 15728880);
					}
				} else {
					if (noMapTextTimer == 0) {
	...
	private static ItemStack getAtlas() {
		...
		if (CompatFlags.CURIOS_LOADED) {
			List<ItemStack> list = CuriosCompat.getCurioAtlases(MC.player);
		...
		return !trinketAtlas.isEmpty() ? trinketAtlas : switch (ModConfigHolder.options().minimapDisplayCondition) {
			...
			case INVENTORY -> {
				...
				if (atlas.isEmpty()) for (ItemStack stack : MC.player.getInventory().items) {
